...
class LoadBalancing:
    ...
    def close_connection(self, connection_id):
        ...
        server=self.connections.pop(connection_id)
        # Close the connection on the server
        server.close_connection(connection_id)
        # Remove the connection from the load balancer
        #optionally in previous just get the connection and then pop it

    def avg_load(self):
        """Calculates the average load of all servers"""
        # Sum the load of each server and divide by the amount of servers
        #improve the code for a real world scenario
        load=0
        servers_amount=len(self.servers)
        if servers_amount>0:
            for server in self.servers:
                load+=load(self)
            load=load/servers_amount
        
        return load

    def ensure_availability(self):
        """If the average load is higher than 50, spin up a new server"""
        #be carefull with the scale, from 10-100, 1-10...
        if self.avg_load()>5
